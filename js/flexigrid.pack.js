	...
	e.addFlex = function (t, n) {
		...
		var r = {
			...
			addData: function (s) {
				...
				var previousCss  = $(this.sDiv).attr("style");
				$(this.sDiv).css({visibility: 'hidden',display:'block'});
				var sHeight=$(this.sDiv).height();
				$(this.sDiv).attr("style", previousCss ? previousCss : "");

				var oo=10;
				var hh=oo+$(this.nDiv).parent().offset().top+$(this.tDiv).height()+$(this.hDiv).height()+$(this.pDiv).height()+sHeight;
				var bb=$(this.bDiv).height();
				var gg=$(window).height();
				if(hh+bb>gg)
					$(this.bDiv).height(gg-hh);
