...
class File_PDF {
     ...
     * @return float
     */
    function getStringWidth($text, $pt = false, $fs = 0)
    {
    	if($fs==0)
    		$fs=($pt ? $this->_font_size_pt : $this->_font_size);
    	else
    		$fs=($pt ? $fs : $fs / ($this->_scale>0?$this->_scale:1));
        $text = (string)$text;
        $width = 0;
        $length = strlen($text);
        for ($i = 0; $i < $length; $i++) {
	...
	*/
	public function setLineStyle($style) 
	{
		...
		if (isset($width)) 
		{
			$width_prev = $this->_line_width;
			$this->setLineWidth($width);
			$this->_line_width = $width_prev;
		}
		if (isset($cap)) 
		{
	...
	*/
    ...
    function setFont($family, $style = '', $size = null, $force = false)
    {
        ...
        $this->_font_size    = $size / ($this->_scale>0?$this->_scale:1);
    ...
    function cell($width, $height = 0, $text = '', $border = 0, $ln = 0,
                  $align = '', $fill = 0, $link = '',$overflow = 1)
    {
		$curFontSize=$this->_font_size_pt;
    	if($this->getStringWidth($text)>$width)
    	{
       		if($overflow==0)
    		{
    			...
    			$newString="";
    			$i=0;
    			while(($i<strlen($text))&&($this->getStringWidth($newString.substr($text,$i,1))<$width))
    			{
	    			$newString.=substr($text,$i,1);
    				$i++;
    			}
    			$text=$newString;
    		}
    		elseif($overflow==2)
    		{	
				$tempFontSize=$curFontSize;
    			while(($this->getStringWidth($text,false,$tempFontSize)>$width)&&($tempFontSize>0))
    				$tempFontSize--;
    			if($tempFontSize>0)
    				$this->setFontSize($tempFontSize);
